    ...
    <div class="mb-6">
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('popular_places_title') }}</h1>
            </div>
        </div>

        <!-- Location Filter -->
        <div class="mb-6">
            <form method="GET" action="{{ route('places.popular') }}" class="flex gap-2">
                <select name="location" 
                        onchange="this.form.submit()"
                        class="flex-1 px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500">
        ...
        <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="p-3">
                            ...
                            @if($place->location)
                                <div class="flex items-center gap-1 text-xs text-gray-500 dark:text-gray-400 mb-2">
                                    ...
                                    <span class="line-clamp-1">{{ $place->location->name }}</span>
                                </div>
                            @endif
            ...
            @empty
                <div class="col-span-2 text-center py-12">
                    ...
                    <p class="text-gray-600 dark:text-gray-400">{{ __('try_different_filter') }}</p>
                </div>
            @endforelse
        </div>
