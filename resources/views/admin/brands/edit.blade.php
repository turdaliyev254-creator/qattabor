<x-admin-layout>
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Edit Brand') }}: {{ $brand->name }}</h2>
    </div>
        ...
        <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $brand->name) }}" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" required>
                    @error('name')
                ...
                @if($brand->icon)
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('Current Icon') }}</label>
                        <img src="{{ asset('images/brands/' . $brand->icon) }}" alt="{{ $brand->name }}" class="w-32 h-32 object-contain rounded-lg border border-gray-300 dark:border-gray-600">
                    </div>
                @endif

                <!-- Icon Upload (Optional for editing) -->
                <div>
                    <label for="icon" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Change Brand Icon') }} {{ __('(Optional)') }}</label>
                    ...
                    <input type="file" name="icon" id="icon" accept="image/svg+xml,image/png,image/jpeg" onchange="previewIcon(this)" class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                    @error('icon')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    ...
                    <div id="iconPreview" class="mt-4 hidden">
                        <p class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('New Icon Preview') }}:</p>
