<x-admin-layout>
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Subcategories</h2>
        <a href="{{ route('admin.subcategories.create') }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow-sm transition-colors duration-200 flex items-center gap-2">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                @if($subcategory->category)
                                    <div class="flex items-center gap-2">
                                        @if($subcategory->category->icon)
                                            <span class="text-lg">{{ $subcategory->category->icon }}</span>
                                        @endif
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <form action="{{ route('admin.subcategories.destroy', $subcategory) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this subcategory?');" class="inline">
                                        @csrf
                                        @method('DELETE')
